...
std::vector<Move> ThetaMoveGenerator::generate(const Board &b) const {
    ...
    for (const auto &pawn: to_be_moved_empty.first) {
        ...
        for (int i = left.length() - 1; i >= 0 && left[i] != '0'; i--) {
            positions.push_back({pawn, Position{i, pawn.row}});
        }
        //Right
        for (int i = 1; i < right.length() && right[i] != '0'; i++) {
            positions.push_back({pawn, Position{pawn.col+i, pawn.row}});
        }
        //Up
        for (int i = up.length() - 1; i >= 0 && up[i] != '0'; i--) {
            positions.push_back({pawn, Position{pawn.col, i}});
        }
        //Down
        for (int i = 1; i < down.length() && down[i] != '0'; i++) {
            positions.push_back({pawn, Position{pawn.col, pawn.row+i}});
        }
    }
